...
func insertSession(t *testing.T, database *gorm.DB, id, channelKey, status, sessionType string, createdAt, updatedAt time.Time) {
	t.Helper()
	sess := &model.Session{
		ID:         id,
		ChannelKey: channelKey,
...
func makeAttachDir(t *testing.T, workspaceDir, sessionType, sessionID string) string {
	t.Helper()
	attachDir := workspace.SessionAttachmentsDir(workspaceDir, sessionType, sessionID)
...
func TestService_CleanApp(t *testing.T) {
	now := time.Now()
	retentionDays := 7
	maxDays := 30
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			insertSession(t, database, sessionID, channelKey, tt.status, model.SessionTypeChat, tt.createdAt, tt.updatedAt)

			if tt.makeAttach {
				makeAttachDir(t, workspaceDir, model.SessionTypeChat, sessionID)
			}
			...
			if tt.wantCount > 0 {
				attachDir := workspace.SessionAttachmentsDir(workspaceDir, model.SessionTypeChat, sessionID)
...
func TestService_Run_MultipleApps(t *testing.T) {
	now := time.Now()
	database := newTestDB(t)
	...
	for _, app := range apps {
		...
		insertSession(t, database, sid, ck, "archived", model.SessionTypeSchedule,
			now.Add(-20*24*time.Hour),
			now.Add(-10*24*time.Hour),
		)
		makeAttachDir(t, app.WorkspaceDir, model.SessionTypeSchedule, sid)
	}
	...
	for _, app := range apps {
		attachDir := workspace.SessionAttachmentsDir(app.WorkspaceDir, model.SessionTypeSchedule, "sess-"+app.ID)
		if _, err := os.Stat(attachDir); !os.IsNotExist(err) {
...
func TestService_CleanApp_WrongApp(t *testing.T) {
	now := time.Now()
	database := newTestDB(t)
	workspaceDir := t.TempDir()

	insertChannel(t, database, "p2p:u1:other-app", "other-app")
	insertSession(t, database, "sess-other", "p2p:u1:other-app", "archived", model.SessionTypeHeartbeat,
		now.Add(-20*24*time.Hour), now.Add(-10*24*time.Hour),
	)
	makeAttachDir(t, workspaceDir, model.SessionTypeHeartbeat, "sess-other")
